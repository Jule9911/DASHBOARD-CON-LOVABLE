...
def load_data():
    ...
    if uploaded_file and uploaded_file != st.session_state.uploaded_file:
        ...
        try:
            ...
            # Agregar timestamp a los datos locales
            df['timestamp'] = datetime.now()
...
def show_real_time_monitoring(current_row, model, preprocessor, feature_columns, target_columns):
    ...
    param_limits = {
        'presion_aceite': {'min': 2, 'ideal': 5, 'max': 7, 'range': [0, 10]},
        'voltaje_bateria': {'min': 10, 'ideal': 13, 'max': 14, 'range': [8, 16]},
        'voltaje_alternador': {'min': 12, 'ideal': 14, 'max': 16, 'range': [10, 18]},
        'temp_vacio': {'min': 50, 'ideal': 70, 'max': 76, 'range': [30, 100]},
        'temp_carga': {'min': 70, 'ideal': 80, 'max': 90, 'range': [50, 120]},
        'nivel_refrigerante': {'min': 0, 'ideal': 0.5, 'max': 1, 'range': [0, 1]}
    }
    ...
    for i, (param, label, unit) in enumerate(params):
        ...
        min_val, ideal, max_val = limits['min'], limits['ideal'], limits['max']

        if param == 'nivel_refrigerante':
            if value >= 1.0:
                estado = "Lleno"
                color = "limegreen"
            elif value == 0:
            ...
            fig_gauges.add_trace(
                go.Indicator(
                    mode="gauge+number",
                    value=value,  # 👈 Línea azul visible
                    title={'text': f"<b style='font-size:13px'>{label}</b>"},
                    number={
                        'prefix': f"<span style='color:{color}; font-size:20px'><b>{estado}</b></span>",
                        'font': {'size': 1},  # oculta valor numérico
                        'valueformat': ''
                    },
                    gauge={
                        'axis': {
                            'range': limits['range'],
                            'tickvals': [0, 0.5, 1],
                            'ticktext': ['Vacío', 'Parcial', 'Lleno'],
                            'tickwidth': 1,
                            'tickcolor': 'black'
                        },
                        'bar': {'color': "#2A5C8A"},  # 👈 Línea azul estilo uniforme
                        'steps': [
                            {'range': [0, 0.5], 'color': "rgba(255, 0, 0, 0.4)"},
                            {'range': [0.5, 1], 'color': "rgba(144, 238, 144, 0.6)"}
                        ],
                        'threshold': {
            ...
            fig_gauges.add_trace(
                go.Indicator(
                    ...
                    value=value,
                    title={'text': f"<span style='font-size:13px'>{label} ({unit})</span>"},
                    number={'font': {'size': 28}},
                    delta={'reference': ideal, 'relative': False, 'font': {'size': 14}},
                    gauge={
                        'axis': {
                            'range': limits['range'],
                            'tickvals': [min_val, ideal, max_val],
                            'ticktext': [f"MIN: {min_val}", f"IDEAL: {ideal}", f"MAX: {max_val}"],
                            'tickwidth': 1,
                            'tickcolor': 'black'
                        },
                        'bar': {'color': "#2A5C8A"},
                        'steps': [
                            {'range': [limits['range'][0], min_val], 'color': "rgba(255, 0, 0, 0.4)"},
                            {'range': [min_val, ideal], 'color': "rgba(144, 238, 144, 0.6)"},
                            {'range': [ideal, max_val], 'color': "rgba(50, 205, 50, 0.6)"},
                            {'range': [max_val, limits['range'][1]], 'color': "rgba(255, 165, 0, 0.4)"}
                        ],
                        'threshold': {
                            'line': {'color': "red", 'width': 4},
                            'thickness': 0.85,
    ...
    st.plotly_chart(fig_gauges, use_container_width=True, config={
        "staticPlot": True,
        "displayModeBar": False
    })
def show_historical_analysis(df):
    """Muestra el análisis histórico con diseño profesional"""
    st.header("📊 Análisis Histórico", divider="blue")
    ...
    if not parametros_disponibles:
        st.error("No se detectaron parámetros válidos en los datos")
        return
    
    # Pestañas con estilo mejorado
    tabs = st.tabs([f"📌 {p.replace('_', ' ').title()}" for p in parametros_disponibles])
    
    for i, param in enumerate(parametros_disponibles):
        with tabs[i]:
            with st.container():
                st.markdown(f'<div class="parameter-title">{param.replace("_", " ").title()}</div>', unsafe_allow_html=True)
                show_single_parameter_analysis(df, param)


def show_single_parameter_analysis(df, parametro):
    """Muestra análisis con diseño profesional"""
    ...
    with st.sidebar.expander(f"⚙️ Configurar {parametro.replace('_', ' ')}", expanded=False):
        try:
            limites = LIMITES.get(parametro, {})
            ideal = st.number_input(
                "Límite ideal",
                value=float(limites.get('ideal', df[parametro].mean())),
                format="%.2f",
                key=f"ideal_{parametro}_unique"  # Clave única
            )
            ruptura = st.number_input(
                "Umbral crítico",
                value=float(limites.get('ruptura', df[parametro].max())),
                format="%.2f",
                key=f"ruptura_{parametro}_unique"  # Clave única
            )
        except Exception as e:
            st.error(f"Error en configuración: {str(e)}")
            return

    # --- Sección de Filtrado con clave única ---
    df_filtrado = df.copy()
    tiene_timestamp = 'timestamp' in df.columns
    ...
    if tiene_timestamp:
        try:
            ...
            # Selector de rango con clave única basada en el parámetro
            col1, col2 = st.columns([3, 1])
            with col1:
                rango = st.date_input(
                    "Seleccionar período",
                    value=(max(min_date, hoy - timedelta(days=3)), min(max_date, hoy)),
                    min_value=min_date,
                    max_value=max_date,
                    format="DD/MM/YYYY",
                    key=f"date_range_{parametro}_unique"  # Clave única por parámetro
                )
            ...
            with col2:
                ...
                if st.button("🔄 Hoy", 
                           ...
                           key=f"btn_hoy_{parametro}_unique"):  # Clave única
                    rango = (hoy, hoy)
            ...
            st.warning(f"Error en filtrado: {str(e)}")
    ...
    with st.container():
        ...
        try:
            current_val = float(df_filtrado[parametro].iloc[-1])
            max_val = float(df_filtrado[parametro].max())
            excedencias = (df_filtrado[parametro] > ruptura).sum() if ruptura else 0
            
            with col1:
                delta_val = current_val - ideal
                color = COLOR_POSITIVO if abs(delta_val) <= (ideal * 0.1) else COLOR_ADVERTENCIA if abs(delta_val) <= (ideal * 0.2) else COLOR_NEGATIVO
                st.metric(
                    "Valor Actual",
                    f"{current_val:.2f}",
                    delta=f"{delta_val:+.2f} vs ideal",
                    delta_color="normal"
                )
            ...
            with col2:
                st.metric(
                    "Máximo Registrado", 
                    f"{max_val:.2f}",
                    help="Valor máximo en el período seleccionado"
                )
            ...
            with col3:
                st.metric(
                    "Excedencias", 
                    excedencias,
                    help="Veces que superó el umbral crítico",
                    delta_color="inverse"
                )
        except Exception as e:
    ...
    if len(df_filtrado) > 0:
        try:
            ...
            fig.add_trace(go.Scatter(
                x=df_filtrado['timestamp'] if tiene_timestamp else df_filtrado.index,
                y=df_filtrado[parametro],
                mode='lines+markers',
                name='Valor',
                line=dict(color='#3498db', width=2.5),
                marker=dict(size=8, color='#3498db', line=dict(width=1, color='#ffffff')),
                hovertemplate="<b>%{x|%d/%m %H:%M}</b><br>Valor: %{y:.2f}<extra></extra>"
            ))
            ...
            if ideal:
                fig.add_hline(
                    y=ideal,
                    line=dict(color=COLOR_POSITIVO, dash='dash', width=2),
                    annotation_text=f"Ideal: {ideal:.2f}",
                    annotation_position="bottom right",
                    annotation_font=dict(color=COLOR_POSITIVO)
                )
            
            if ruptura:
                fig.add_hline(
                    y=ruptura,
                    line=dict(color=COLOR_NEGATIVO, dash='dot', width=2),
                    annotation_text=f"Crítico: {ruptura:.2f}",
                    annotation_position="top right",
                    annotation_font=dict(color=COLOR_NEGATIVO)
                )
            ...
            fig.update_layout(
                ...
                yaxis=dict(title=parametro.replace('_', ' ').title())
            )
    ...
    tab1, tab2, tab3 = st.tabs(["Estadísticas", "Distribución", "Excepciones"])
    
    with tab1:
        if len(df_filtrado) > 0:
            stats = df_filtrado[parametro].describe().to_frame().T
            st.dataframe(
                stats.style.format("{:.2f}")
                .background_gradient(cmap='Blues')
                .highlight_max(color='#f39c12'),
                use_container_width=True
            )
        ...
        if len(df_filtrado) > 0:
            ...
            with col1:
                fig_hist = px.histogram(
                    df_filtrado, x=parametro, nbins=20,
                    color_discrete_sequence=['#3498db'],
                    title="Distribución de Frecuencia"
                )
                if ideal: fig_hist.add_vline(x=ideal, line_color=COLOR_POSITIVO)
                if ruptura: fig_hist.add_vline(x=ruptura, line_color=COLOR_NEGATIVO)
                st.plotly_chart(fig_hist, use_container_width=True)
            
            with col2:
                fig_box = px.box(
                    df_filtrado, y=parametro,
                    color_discrete_sequence=['#3498db'],
    ...
    with tab3:
        if ruptura and len(df_filtrado) > 0:
            df_excedencias = df_filtrado[df_filtrado[parametro] > ruptura]
            if len(df_excedencias) > 0:
                st.dataframe(
                    df_excedencias.sort_values(parametro, ascending=False),
                    column_config={
                        "timestamp": st.column_config.DatetimeColumn("Fecha/Hora"),
                        parametro: st.column_config.NumberColumn("Valor", format="%.2f")
                    },
                    use_container_width=True
                )
